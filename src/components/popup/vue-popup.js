exports.install = function (Vue, options) {
  ...
  // 最简单的一句提示语，2秒消失
  Popup.prototype.tips = function(title,callback,position){
  ...
  // 提示不消失框
  Popup.prototype.tipsContinue = (title,time,callback)=>{
    var ele = document.createElement("div");
    ele.className = 'popup-tip middle';
    ele.innerHTML = '<div class="title">'+title+'</div>';
    document.body.appendChild(ele);
    if(time){
      setTimeout(() =>{
  ...
  Popup.prototype.show = function(opt){
    ...
    for(var i = 0, len=opt.btn.length; i<len; i++){
      btn += '<a class="popup-btn col" data-index="'+i+'" style="width:'+100/len+'%">'+opt.btn[i].name+'</a>'
    }
    btn += '</div>'
    ...
    +btn+
    '</div>';
    var element = document.body.getElementsByClassName('popup-box');
    if(element.length > 0){
